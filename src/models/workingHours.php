...
class WorkingHours extends Model {
    ...
    public function getBalance() {
        ...
        if($this->worked_time == DAILY_TIME) return '00';
    ...
    public static function getAbsentUsers() {
        $today = new DateTime();
        $result = Database::gerResultFromQuery("SELECT name FROM users WHERE end_date IS NULL AND id NOT IN (SELECT user_id FROM working_hours WHERE work_date = '{$today->format('Y-m-d')}' AND time1 IS NOT NULL)");
    ...
    public static function getWorkedTimeInMonth($yearAndMonth) {
        ...
        $resul = static::getResultSetFromSelect([
            'raw' => "work_date BETWEEN '{$startDate}' AND '{$endDate}'"
        ], "sum(worked_time) AS sum");

        return $resul->fetch_assoc()['sum'];
    }
