	...
	<div>
		<form action="#" method="post" class="mb-4">
			<div class="input-group">
				...
				<select name="user" class="form-control mr-3" placeholder="Selecione o usuário..." >
					<?php
						foreach($users as $user) {
							$selected = $user->id === $selectedUserId ? 'selected' : '';
							$muted = $user->end_date ? 'class="text-muted"' : '';
							echo "<option {$muted} value='{$user->id}' {$selected} >{$user->name}</option>";
						}	
					?>
				</select>
		...
		<table class="table table-bordered table-striped table-hover">
			...
			<tbody>
				<?php foreach($report as $registry): ?>
					<tr>
						<td><?= formatDateWithLocalde($registry->work_date, '%A, %d de %B de %Y'); ?></td>
